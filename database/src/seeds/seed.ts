...
async function seedPackages() {
  ...
  const pkgs = [
    {
      ...
      colorTheme: '#6B7280',
      sortOrder: 1,
      highlights: DEFAULT_PACKAGE_HIGHLIGHTS.basic,
      isRecommended: false,
    },
    {
      slug: 'standard',
      ...
      colorTheme: '#3B82F6',
      sortOrder: 2,
      highlights: DEFAULT_PACKAGE_HIGHLIGHTS.standard,
      isRecommended: false,
    },
    {
      slug: 'premium',
      ...
      colorTheme: '#8B5CF6',
      sortOrder: 3,
      highlights: DEFAULT_PACKAGE_HIGHLIGHTS.premium,
      isRecommended: true,
    },
    {
      slug: 'luxury',
      ...
      colorTheme: '#F59E0B',
      sortOrder: 4,
      highlights: DEFAULT_PACKAGE_HIGHLIGHTS.luxury,
      isRecommended: false,
    },
  ];
  ...
  const existingPPs = await db.select().from(packagePrices);
  for (const p of prices) {
    const existing = existingPPs.find(ex => ex.packageId === p.packageId && !ex.effectiveTo);
    if (existing) {
      await db.update(packagePrices).set({
        pricePerSqft: p.pricePerSqft,
        volumeDiscountThresholdSqft: p.volumeDiscountThresholdSqft,
        volumePricePerSqft: p.volumePricePerSqft,
      }).where(eq(packagePrices.id, existing.id));
    } else {
      await db.insert(packagePrices).values(p);
    }
  }
...
async function seedAddons() {
  ...
  for (const a of addonDefs) {
    await db.insert(addons).values(a).onConflictDoUpdate({
      ...
      set: {
        ...
        allowsMultiple: a.allowsMultiple ?? false,
  ...
  const addonPriceDefs = [
    ...
    // 13. Motor Automation: ₹12,000 (bore), ₹12,000 (corporation) - multi-select enabled
  ...
  // Clean up legacy 'both' variant if it was previously seeded
  const motorAddonId = addonIdBySlug['motor_automation'];
  if (motorAddonId) {
    await db.delete(addonPrices).where(and(eq(addonPrices.addonId, motorAddonId), eq(addonPrices.variantSlug, 'both')));
  }
