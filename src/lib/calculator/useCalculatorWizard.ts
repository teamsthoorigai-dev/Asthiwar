...
export function useCalculatorWizard() {
  ...
  useEffect(() => {
    ...
    Promise.all([getLocations(), getPackages()])
      .then(([locs, pkgs]) => {
        if (!mounted) return;
        const uniquePkgs = Array.from(new Map(pkgs.map((p) => [p.slug, p])).values());
        const uniqueLocs = Array.from(new Map(locs.map((l) => [l.slug, l])).values());
        setLocations(uniqueLocs);
        setPackages(uniquePkgs);
        if (uniqueLocs.length > 0) {
          setFormData((prev) => ({
            ...prev,
            plotLocation: prev.plotLocation || uniqueLocs[0].name,
            locationId: prev.locationId || uniqueLocs[0].id,
          }));
        }
        setLoading(false);
      })
  ...
  useEffect(() => {
    ...
    const loadConfig = async () => {
      ...
      try {
        ...
        if (mounted && cfg) {
          const deduplicatedAddons = (cfg.addons || []).map((addon) => ({
            ...addon,
            variants: Array.from(new Map(addon.variants.map((v) => [v.variantSlug, v])).values()),
          }));
          const deduplicatedSpecs = (cfg.specifications || []).map((cat) => ({
            ...cat,
            items: (cat.items || []).map((item) => ({
              ...item,
              options: Array.from(new Map(item.options.map((opt) => [opt.slug, opt])).values()),
            })),
          }));
          setPackageConfig({
            ...cfg,
            addons: deduplicatedAddons,
            specifications: deduplicatedSpecs,
          });
        }
      } catch (err) {
